---
description: Automatically detect the language, framework, package manager, test runner, linter, and build tool used in the current project. Produces an autoimprove.config.md file that the improve loop uses as its measurement suite. Run this once before starting the improve loop in a new project.
---

# Detect Stack Skill

Analyse the project root and produce a `autoimprove.config.md` that configures the measurement suite for this specific codebase.
...
## Step 4 — Write autoimprove.config.md
...
Measurement suite written to autoimprove.config.md.
